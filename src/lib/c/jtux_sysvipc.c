...
static bool field_jtoc_perm(JNIEnv *env, jclass cls, const char *field, jobject obj, struct ipc_perm *perm)
{
	jfieldID fid;
	...
	if (!field_jtoc_uid(env, clsperm, "uid", objperm, &perm->uid))
		return false;
	if (!field_jtoc_gid(env, clsperm, "gid", objperm, &perm->gid))
		return false;
	if (!field_jtoc_uid(env, clsperm, "cuid", objperm, &perm->cuid))
		return false;
	if (!field_jtoc_gid(env, clsperm, "cgid", objperm, &perm->cgid))
		return false;
...
JNIEXPORT void JNICALL Java_jtux_USysVIPC_msgctl(JNIEnv *env, jclass obj, jint msqid, jint cmd, jobject data)
{
...
JNIEXPORT void JNICALL Java_jtux_USysVIPC_msgctl2(JNIEnv *env, jclass obj, jint msqid, jint cmd, jobject data)
{
    if(cmd != MSG_INFO) {
        return;
    }

	struct msginfo data_c;
	jclass cls = (*env)->FindClass(env, "jtux/USysVIPC$s_msginfo");
	int r;

	memset(&data_c, 0, sizeof(data_c));
    // NOTE cast is required as noted in the man page for msgctl.  See man msgctl and read the msginfo part.
	JTHROW_neg1(r = msgctl(msqid, cmd, (struct msqid_ds *) &data_c))
	if (r == -1) 
		return;
	if (cmd == MSG_INFO) {
		if (!field_ctoj_int(env, cls, "msgpool", data, data_c.msgpool))
			return;
		if (!field_ctoj_int(env, cls, "msgmap", data, data_c.msgmap))
			return;
		if (!field_ctoj_int(env, cls, "msgmax", data, data_c.msgmax))
			return;
		if (!field_ctoj_int(env, cls, "msgmnb", data, data_c.msgmnb))
			return;
		if (!field_ctoj_int(env, cls, "msgmni", data, data_c.msgmni))
			return;
		if (!field_ctoj_int(env, cls, "msgssz", data, data_c.msgssz))
			return;
		if (!field_ctoj_int(env, cls, "msgtql", data, data_c.msgtql))
			return;
		if (!field_ctoj_short(env, cls, "msgseg", data, data_c.msgseg))
			return;
	}
}
